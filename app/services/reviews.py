...
def latest_review_name() -> str | None:
    ...
    if not review_dir or not review_dir.exists():
        return None
    reviews = sorted(review_dir.glob("*-review.md"), reverse=True)
    return reviews[0].name if reviews else None


def read_latest_review() -> tuple[str, str] | None:
    """Return (filename, content) for the latest platform review, or None."""
    review_dir = _review_dir()
    if not review_dir or not review_dir.exists():
        return None
    reviews_list = sorted(review_dir.glob("*-review.md"), reverse=True)
    if not reviews_list:
        return None
