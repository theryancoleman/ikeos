...
## 2026-07-16: Scheduler leader election via advisory file lock (fixes 2026-07-16 desync bug above)

Fixed the gunicorn `--workers 2` vs. single-worker scheduler conflict without reverting to `--workers 1`. `app/services/scheduler.py` now does non-blocking `fcntl.flock()` leader election against `/tmp/ikeos-scheduler.lock` (container-local, not a mounted volume) at `start()` time; only the winning worker process creates a live `BackgroundScheduler` and owns the `housekeeping` cron job — the losing worker(s) run no competing scheduler, eliminating double-firing. `next_run` reporting no longer depends on any live scheduler instance at all: `get_config_with_next_run()` computes it analytically from `schedule.json`'s cron fields via `apscheduler.triggers.cron.CronTrigger.get_next_fire_time()`, so every worker (leader or not) returns the identical value for `GET /housekeeping/schedule` — no persistence or cross-process sync needed for reads. For writes, `update_config()` still calls `_apply_to_live_scheduler()` immediately (no-op on non-leader workers), and the leader additionally runs a `housekeeping-sync` interval job (every 60s) that re-reads `schedule.json` and reapplies it to the live cron trigger if changed — this closes the gap where a `PATCH` lands on a non-leader worker and would otherwise never reach the leader's actual firing job until restart. `trigger_now()` ("Run Now") was already leader-independent (calls `run_scheduled_housekeeping()` in-process) and needed no change. Net effect: gunicorn can run any worker count safely; the single-worker assumption from 2026-06-18 is retired.
...
## 2026-07-02: Driver model resolved — the session-manager HTTP contract IS the driver interface (supersedes "pluggable driver model planned for v0.2")

Outcome of the dedicated architecture session (Fable 5 review). No `DriverBase` Python protocol will be built. The driver abstraction is the session-manager HTTP API, to be documented as `docs/SESSION_DRIVER_API.md` v0: `POST /sessions` shapes, 409-as-dedup, name-as-identity, the `initial_command`-implies-ephemeral-implies-permission-skipping rule, and an optional opaque `model` field. Anyone can implement the contract to drive a different engine — that is the multi-driver story at zero Python-API cost. Consolidation work (all traffic through `session_client.py`, intent-level `driver.py` owning every slash-command string) is planned in `docs/superpowers/plans/2026-07-02-driver-consolidation.md` on branch `feat/driver-consolidation`. Undo cost if the contract proves too Claude-shaped: version the HTTP API (v1) — far cheaper than deprecating a public Python protocol.

## 2026-07-02: Skills and session-manager move INTO ikeos as adapters/claude-code/ (supersedes "skill implementations stay in claude-config" and "docs/skills/ reference planned")

Second outcome of the same session. The IkeOS-coupled skills (/housekeeping, /triage, /close-session, /schema-check, /promote + reflection hook) become installable adapter content in this repo at `adapters/claude-code/skills/`, parameterized by env (IKEOS_URL, CAPTURE_TOKEN, VAULT_PATH) — working integration, not copy-adapt docs, versioned in lockstep with the capture API. The session-manager service also moves here (`adapters/claude-code/session-manager/`) as the reference implementation of the driver contract — otherwise public IkeOS's flagship features depend on invisible private code. No separate ikeos-claude repo (git filter-repo preserves the escape hatch). claude-config stays 100% private with no public starter twin. HARD GATE before any extraction: rotate the capture token and sweep both repo histories (tracked in claude-config vault, priority high). Full rationale: vault decisions/ 2026-07-02 (three ADR drafts, status proposed).

## 2026-07-04: AIOS Engineering Standard lives in IkeOS as the authoritative source

`docs/engineering/CLEAN_CODE_FOR_AIOS.md` is the canonical AIOS engineering standard. It is versioned here (not in claude-config or per-project) because IkeOS is the platform repo. Other AIOS projects reference it; they do not duplicate it. The `/code-review` skill at `adapters/claude-code/skills/code-review.md` reads this file at invocation time, so updating the standard automatically improves the review without changing the skill. This is the rubric for the eventual automated review integration.

## 2026-07-17: reviews.py rewired to the platform-review-narrative output convention (docs/platform-reviews/*-review.md)
...
## 2026-08-17: PATCH /entries gains a scoped project-relocation capability — the second sanctioned vault-write door beyond status/tags

Extended the capture API's `PATCH /entries` to accept an optional `new_project` field, calling `relocate_entry_project()` — moves an entry's file to a different project's folder and corrects both its `project` frontmatter field and its project tag. Motivation: a claude-config housekeeping task was failing because 18+ vault entries had `project: visualizer` (lowercase) instead of the canonical `Visualizer` (the folder's real, capitalized name), and there was no sanctioned way to correct a `project` field — the existing PATCH only ever touched `status`/`updated`/the `status/*` tag, and direct vault file writes are permission-denied to agents by design (Vault Access Rules — "no third door"). This is a second, narrowly-scoped door: `new_project` only relocates/corrects the project field, nothing else. A same-file guard (via `Path.samefile`) rewrites in place instead of deleting when source and destination resolve to the same physical file (case-only corrections on the case-insensitive NTFS-backed vault mount), and a destination-collision guard refuses the move rather than overwriting an existing entry — both were caught as Critical data-loss bugs in a final whole-branch review before merge, since the endpoint's own first real-world use case (correcting the 18 Visualizer entries) would have triggered the same-file path immediately. `new_project` is validated against `get_projects()` (the real on-disk project list), not just path-traversal characters, so a typo can't silently create a phantom project folder.
...
## 2026-08-18: council-item is a new entry type with its own status lifecycle, token- and capability-gated routes, and its own blueprint

The council-pipeline-foundation plan adds `council-item` to `ENTRY_TYPE_CONFIG` (`app/services/vault_cache.py`) with its own status lifecycle — `pending-review → in-discussion → approved/declined/actioned` — rather than reusing `idea`'s `new/open/in-progress/done/deferred` states. Council items are AI/human-review-workflow items (weekly narrative review candidates awaiting a Discuss/Approve/Decline decision) with fundamentally different semantics than idea backlog triage; mixing the two lifecycles would confuse `/triage` (which filters on `status: new`) and dashboard idea-counts (which would either double-count council items as backlog ideas or need extra filtering to exclude them). Same pattern precedent as the 2026-06-27 experiment-status-lifecycle decision.

`app/routes/council.py` is a new blueprint, not an addition to `housekeeping.py`, mirroring the 2026-07-17 research_sources.py precedent: a new concern gets its own blueprint rather than growing an already-large file that owns a different concern.

The three council routes (`/council/<slug>/discuss`, `/approve`, `/decline`) are both token-gated (`@require_capture_token`) and capability-gated (`council_pipeline`, defaults disabled) — a stricter posture than research_sources.py's routes, which have no token auth at all (per that file's own 2026-07-17 DECISIONS.md entry, because they only proxy to the session-manager's own unauthenticated endpoints with no local vault write to protect). Council routes are different: `discuss`/`approve` mutate council-item status directly in the vault *and* trigger real Claude Code sessions (a moderator session for discuss, an implementation session in a worktree for approve) — real side effects that warrant both layers of gating rather than either alone.

This is Phase 1 of a larger plan. Two follow-ons build on this backend contract and are the actual consumers of this API: a claude-config-repo plan (narrative task rewiring to create council-items with slug-matching/aging, the `/council-discuss` and `/council-action` moderator/implementer skills, `/platform-review` unification) and an IkeOS UI plan (the `/housekeeping` dashboard Council widget — pending count, weeks-open badges, Discuss/Approve/Decline buttons). Both were deliberately deferred rather than bundled into this plan so the API/service-layer work here could be independently reviewable and mergeable, and so the follow-on UI plan could be written against the real, tested API shape instead of an assumed one.
...
## 2026-08-20: Adapter/harness drift consolidation — copy-and-regenerate mechanism (Passes 1–2 complete, housekeeping.md deferred to Pass 3)

Resolves the drift named in the 2026-08-19 entry above, per `docs/superpowers/specs/2026-08-19-adapter-harness-consolidation-design.md`. `ikeos/adapters/claude-code/{skills,session-manager}/` is now the single source of truth; `claude-config/scripts/sync.sh generate` regenerates the deployed copies (`claude-config/global/commands/*.md`, `claude-config/services/session-manager/*.py`+`start.sh`), each stamped with a "do not edit here, generated from ikeos" header, and `claude-config/scripts/check-drift.sh` auto-heals both boundaries at every `SessionStart` — the same pattern the original `global/` → `~/.claude` check already used, extended one hop earlier in the chain.

**Pass 1 (skills):** reconciled `triage`, `promote`, `schema-check`, `close-session` (four of five duplicated skills; `housekeeping.md` excluded — see below). Root-caused part of the drift to a missing `env` block on the live `~/.claude/settings.json`, fixed there and made durable via `bootstrap-wsl2.sh`. `close-session.md`'s Section 5a blog-notes divergence was investigated rather than assumed: the adapter's Python "set" branch had a real bug (computed the target path but never wrote to it), fixed rather than reverting to the deployed copy's non-executable template. Incidentally found and fixed a `find`-without-`-L` bug in `sync.sh`'s orphan-detection/removal (`show_diff`/`do_apply`) — `~/.claude/commands` resolves through a symlink onto the Windows drvfs mount, where bare `find -type f` silently returns nothing, so deleted source files were never actually cleaned up from the deployed copy.

**Pass 2 (session-manager):** before writing a plan, `git log --all` turned up an already-complete but unmerged branch (`feat/session-manager-adapter-sync`, 2026-07-21/22, referenced in the 2026-07-22 entry below) that had done most of this work — model-parameter threading, `list_session_names()`, `research_sources.py` + its three routes, `_reconcile_sessions()` — while deliberately keeping the adapter's env-var-based `CLAUDE_BIN`/`CLAUDE_PLUGIN_BASE` config instead of the deployed service's hardcoded host paths. Recovered via merge (not fast-forward — `main` had 45 unrelated commits since the branch's base, none touching the session-manager path, so a real three-way merge was needed; a first attempted `--ff-only` failed and was corrected before any data loss). Reconciled the further drift that accumulated on deployed *after* that branch: `parse_stuck_on_menu()` (a 2026-08-17 incident fix), the `/sessions/<id>/keys` raw-key endpoint, a defensive `session.get("remote_control", False)` read. Kept the branch's atomic TOCTOU-safe `add_source()` rather than copying deployed's non-atomic pre-check, which was a real regression (deployed's version predates the branch's fix). Made `research_sources.py`'s storage path env-var overridable (`RESEARCH_SOURCES_PATH`) rather than picking one hardcoded path for both sides — deployed's `library/research-sources.json` location is a genuine, intentional integration point (other `claude-config` tooling reads it directly), not incidental drift. Reaffirmed (via an empty commit, deliberately) the branch's original decision not to hardcode a `DEFAULT_MODEL` fallback in the adapter — a specific default model is a this-host preference, not reference-implementation material; the one accepted behavior change is that sessions created without an explicit `model` field now store `null` in their session record instead of `"claude-sonnet-5"` (the actual tmux launch command is unaffected, since that string is still the hardcoded literal in `CLAUDE_CMD` either way). The live service was restarted via the existing `restart.sh` and verified with a real session create/list/delete cycle and a `/research-sources` data-identity check, not just its test suite — this incidentally moved the live process off the tmux-wrapped launch method (`setup-startup.ps1`'s Task Scheduler action, which contradicts `start.sh`'s own `unset TMUX` isolation-fix comment) onto a properly detached one, though `setup-startup.ps1` itself was left unchanged (Windows-only ops tooling, out of scope).

**Deferred to Pass 3, no date set:** `housekeeping.md` and the council-pipeline skills (`council-discuss.md`/`council-action.md`, deployed-only). The deployed `housekeeping.md` has grown a task dependency-chain system and full council-pipeline integration absent from the adapter — real, actively-evolving production functionality (see the 2026-08-18 council-item entry above), not incidental drift. Porting it into the "stable portable reference" now risks the adapter going stale again almost immediately; revisit once the council pipeline stabilizes.
