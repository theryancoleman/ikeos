...
## 7. Duplication / drift points (resolved for skills and session-manager; housekeeping.md deferred)

Resolved 2026-08-19/20 via the adapter/harness drift consolidation (`docs/superpowers/specs/2026-08-19-adapter-harness-consolidation-design.md`, Passes 1–2; see `.claude/DECISIONS.md` for the full record). `ikeos/adapters/claude-code/{skills,session-manager}/` is now the single source of truth for four skills (`triage`, `promote`, `schema-check`, `close-session`) and the session-manager service; `claude-config/scripts/sync.sh generate` regenerates the deployed copies (`claude-config/global/commands/*.md`, `claude-config/services/session-manager/*.py`+`start.sh`) from that source, each stamped with a "do not edit here" header. `claude-config/scripts/check-drift.sh` now watches both `ikeos/adapters/` boundaries (skills and session-manager) in addition to its original `claude-config/global/` → `~/.claude` check, auto-healing drift at every `SessionStart`.

**One remaining open item:** `housekeeping.md` (and the council-pipeline skills `council-discuss.md`/`council-action.md`, deployed-only, no adapter counterpart) are explicitly deferred — the deployed `housekeeping.md` has grown real, actively-evolving production functionality (a task dependency-chain system and full council-pipeline integration) absent from the adapter, and porting it into the "stable portable reference" now risks the adapter going stale again almost immediately. This is Pass 3 of the consolidation, with no date set, blocked on the council pipeline stabilizing. `code-review.md` remains adapter-only by design (no deployed counterpart) — a one-way asymmetry, not drift.
