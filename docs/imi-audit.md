...
**Action:** After the housekeeping permission bug is fixed (area 10), define how a scheduled job reports its completion state — whether that is a structured log entry, a vault write, or a new endpoint is an open design question. The key requirement is that the next session can inspect whether the previous scheduled run succeeded and what it did. Defer implementation until the bug fix reveals what completion state is naturally available.
...
- `app/static/bundle.css` — generated by `scripts/bundle_css.py` at build time; committing it pollutes diffs and creates a "last build output" confusion for contributors
- `.claude/settings.local.json` — personal permission overrides; should never be in source control
- `venv/` is listed in `.gitignore` but may still be tracked if files were committed before the rule was added
...
**Action:** Add `app/static/bundle.css` and `.claude/settings.local.json` to `.gitignore`. Verify whether `venv/` is tracked before acting: `git ls-files --error-unmatch venv/` — if it exits 0 (tracked), run `git rm -r --cached venv/`; if it exits non-zero (not tracked), no action needed. This is Task 5.
...
- **Session continuity for scheduled runs** — After the housekeeping permission bug is fixed, define how a scheduled job reports completion state (log, vault write, or endpoint — open design question). The requirement: the next session can inspect whether the previous run succeeded.
